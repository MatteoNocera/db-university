...
SELECT 
`students`.`id`, `students`.`name`, `students`.`surname`, `courses`.`id`, `courses`.`name`, COUNT(DISTINCT `exam_student`.`exam_id`) AS 'numberOfTry', MAX(`exam_student`.`vote`) AS 'maxVote'
FROM `students`
